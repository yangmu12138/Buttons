...
class OrderSettings:
    def __init__(self, root):
        self.root = root
        self.root.title("Order Settings")
        self.root.geometry("600x400")
        self.root.configure(bg="#f0f0f0")
        self.select_label_list=["Order"]
    ...
    def create_widgets(self):
        ...
        # 创建输入框容器
        inputs_frame = tk.Frame(self.root, bg="#f0f0f0")
        inputs_frame.pack(pady=10)
        
        # 输入框配置
        input_configs = [
            {"label": "Move_forward_order:", "default": "select the order"},
            {"label": "Move_backward_order:", "default": "select the order"},
            {"label": "Steer_number:", "default": "select the number"},
            {"label": "Steer_order:", "default": "select the order"}
        ]
        
        # 端口选项
        self.move_forward_options = ["mf+t", "move_forward+t"]
        self.selected_ports = []
        self.move_back_options=["mb+t","move_backward+t"]
        self.steer_number_options=["3","4","5","6","7"]
        self.steer_command_options = ["d+number+angle","steer+number+angle"]
        self.selected_forward_commands = []
        self.selected_backward_commands = []
        
        
        for i, config in enumerate(input_configs):
            frame = tk.Frame(inputs_frame, bg="#f0f0f0")
            frame.pack(pady=15, fill=tk.X, padx=40)
            ...
            if config["label"] == "Move_forward_order:":
                # 创建下拉式多选框
                self.create_multi_select_dropdown(frame, "orders", self.move_forward_options, "selected_ports")
            elif config["label"] == "Move_backward_order:":
                # 创建MCU端口多选框
                self.create_multi_select_dropdown(frame, "orders", self.move_back_options, "selected_ports")
            elif config["label"] == "Steer_number:":
                # 创建下拉式多选框
                self.create_multi_select_dropdown(frame, "numbers", self.steer_number_options, "selected_ports")
            else:
                # 创建其他端口多选框
                self.create_multi_select_dropdown(frame, "orders", self.steer_command_options, "selected_ports")
        
        # 创建保存按钮
        save_button_frame = tk.Frame(self.root, bg="#f0f0f0")
        save_button_frame.pack(side=tk.BOTTOM, anchor=tk.SE, padx=10, pady=10)
        
        save_button = self.create_circular_button(save_button_frame, "保存", "#4CAF50", self.save_settings, size=60)
        save_button.pack()
    ...
    def create_circular_button(self, parent, text, color, command, size=60):
        ...
        # 绑定点击事件
        canvas.bind("<Button-1>", lambda e: [self.button_command(self.select_label_list),self.shut_down_page()])
    ...
    def shut_down_page(self):
        """关闭当前页面"""
        self.root.destroy()
def order_setting_start():
    root = tk.Tk()
    app = OrderSettings(root)
    root.mainloop()
