...
class PortSettings:
    def __init__(self, root):
        self.root = root
        self.root.title("Port Settings")
        self.root.geometry("600x400")
        self.root.configure(bg="#f0f0f0")
        self.select_label_list=["Port"]
        self.create_widgets()
    ...
    def create_widgets(self):
        ...
        # 输入框配置
        input_configs = [
            {"label": "Camera_Ports(端口):", "default": "input the port"},
            {"label": "MCU_Ports(端口):", "default": "input the port"},
            {"label": "Other_Ports(端口):", "default": "input the port"}
        ]
        
        # 端口选项
        self.port_options = ["9090","9091","9097"]
        self.selected_ports = []
        
        # 命令选项
        self.command_options = ["8087", "8089", "8091", "8097","8089"]
        ...
        for i, config in enumerate(input_configs):
            ...
            if config["label"] == "Camera_Ports(端口):":
                # 创建下拉式多选框
                self.create_multi_select_dropdown(frame, "ports", self.port_options, "selected_ports")
            elif config["label"] == "MCU_Ports(端口):":
                # 创建MCU端口多选框
                self.create_multi_select_dropdown(frame, "ports", self.command_options, "selected_ports")
            else:
                # 创建其他端口多选框
                self.create_multi_select_dropdown(frame, "ports", self.port_options, "selected_ports")
        ...
        save_button_frame.pack(side=tk.BOTTOM, anchor=tk.SE, padx=20, pady=20)
    ...
    def create_multi_select_dropdown(self, parent, title, options, selected_var):
        ...
        # 添加选项
        vars_dict = {}
       # vars_list=["camera_ports","mcu_ports","other_ports"]
    ...
    def toggle_option(self, option, var, selected_label, selected_var, options,vars_dict):
        ...
        #self.select_label_list.append(vars_list)
...
def port_setting_start():
    root = tk.Tk()
    app = PortSettings(root)
    root.mainloop()
